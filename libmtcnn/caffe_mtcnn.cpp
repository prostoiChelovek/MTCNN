...
int caffe_mtcnn::run_PNet(const cv::Mat& img,
		scale_window& win, std::vector<face_box>& box_list)
{
	cv::Mat  resized;
	int scale_h=win.h;
	int scale_w=win.w;
	...
	generate_bounding_box(confidence->cpu_data(),
			confidence->count(), reg->cpu_data(),
			scale, pnet_threshold_, feature_h,
...
void caffe_mtcnn::copy_one_patch(const cv::Mat& img,
		face_box&input_box,float * data_to, int width, int height)
{
	...
	cv::copyMakeBorder(chop_img, chop_img, pad_top,
