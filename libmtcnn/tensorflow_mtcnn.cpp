...
void tf_mtcnn::run_PNet(const cv::Mat &img, scale_window &win, std::vector<face_box> &box_list) {
    ...
    TF_Tensor *input_tensor = TF_NewTensor(TF_FLOAT, dim, 4, resized.ptr(), sizeof(float) * scale_w * scale_h * 3,
    ...
    const auto *conf_data = (const float *) TF_TensorData(output_values[1]);
    const auto *reg_data = (const float *) TF_TensorData(output_values[0]);
...
void tf_mtcnn::run_ONet(const cv::Mat &img, std::vector<face_box> &rnet_boxes, std::vector<face_box> &output_boxes) {
    ...
    const auto *conf_data = (const float *) TF_TensorData(output_values[2]);
    const auto *reg_data = (const float *) TF_TensorData(output_values[0]);
    const auto *points_data = (const float *) TF_TensorData(output_values[1]);
